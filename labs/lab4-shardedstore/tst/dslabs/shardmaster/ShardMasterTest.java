...
public class ShardMasterTest extends DSLabsJUnitTest {
  ...
  private Set<Integer> fullShardRange(int numShards) {
    return IntStream.rangeClosed(1, numShards).boxed().collect(Collectors.toSet());
  }
  ...
  private ShardConfig getConfig(int configNum, boolean checkIsNext, boolean checkFresh) {
  ...
  private void checkConfig(int numMoved, int numShards, ShardConfig config, Integer... groupIds) {
    ...
    assertEquals(new HashSet<>(Arrays.asList(groupIds)), config.groupInfo().keySet());
  ...
  private void checkShardMovement(ShardConfig previous, ShardConfig current, int numShards) {
    assertEquals(previous.configNum() + 1, current.configNum());

    int numMoved =
        previous.groupInfo().keySet().stream()
            .mapToInt(
                gid -> {
                  Set<Integer> p = new HashSet<>(previous.groupInfo().get(gid).getRight());
    ...
    int previousNumGroups = previous.groupInfo().size(),
        currentNumGroups = current.groupInfo().size();
    ...
    if (previousNumGroups < currentNumGroups) {
      int newGroup =
          current.groupInfo().keySet().stream()
              .filter(gid -> !previous.groupInfo().containsKey(gid))
              .findAny()
              .orElseThrow(AssertionError::new);
      assertEquals(current.groupInfo().get(newGroup).getRight().size(), numMoved);
      assertEquals(numShards / current.groupInfo().size(), numMoved);
    } else if (currentNumGroups < previousNumGroups) {
      int removedGroup =
          previous.groupInfo().keySet().stream()
              .filter(gid -> !current.groupInfo().containsKey(gid))
              .findAny()
              .orElseThrow(AssertionError::new);
      assertEquals(previous.groupInfo().get(removedGroup).getRight().size(), numMoved);
    } else {
  ...
  public void test01commandsReturnOk() {
    ...
    int shardToMove =
        config.groupInfo().get(1).getRight().stream()
            .findAny()
            .orElseThrow(() -> new AssertionError("Group 1 has no shards mapped to it"));
  ...
  public void test04InitialConfigCorrect() {
    execute(new Join(1, group(1)));
    Map<Integer, Pair<Set<Address>, Set<Integer>>> expected = new HashMap<>();
    expected.put(1, new ImmutablePair<>(group(1), fullShardRange(DEFAULT_NUM_SHARDS)));
  ...
  public void test07MoveShards() {
    ...
    for (Integer shard : groupOneShards) {
      ...
      checkConfig(groupOneShards.size() - remaining.size(), DEFAULT_NUM_SHARDS, config, 1, 2);
