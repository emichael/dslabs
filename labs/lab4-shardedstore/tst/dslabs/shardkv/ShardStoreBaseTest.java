...
public abstract class ShardStoreBaseTest extends BaseJUnitTest {
    ...
    builder.serverSupplier(
        a -> {
          if (Arrays.asList(shardMasters).contains(a)) {
            return new PaxosServer(a, shardMasters.clone(), new ShardMaster(numShards));

          } else {
            Matcher m = Pattern.compile("server(\\d+)-(\\d+)").matcher(a.toString());
            ...
            if (m.find()) {
              ...
              return new ShardStoreServer(
                  a, shardMasters.clone(), numShards, group.clone(), groupId);
            }

            throw new RuntimeException("Can't create server for given address.");
          }
        });

    builder.clientSupplier(
        a -> {
          if (a.equals(CCA)) {
  ...
  static void addServers(
      AbstractState state, int numGroups, int numServersPerGroup, int numShardMasters) {
  ...
  void joinGroup(int groupNum, int numServersPerGroup) throws InterruptedException {
    sendCommandAndCheck(
        configController, new Join(groupNum, servers(groupNum, numServersPerGroup)), new Ok());
  }
  ...
  void assertConfigBalanced() throws InterruptedException {
    ...
    Integer max =
        config.groupInfo().values().stream()
  ...
  /**
   * Runs a single-client, single-group search. The client should be client(1). Must call
   * setupStates(1, 1, 1, N) before calling this method.
   */
  void singleClientSingleGroupSearch() {
    initSearchState.addClientWorker(
        CCA, Workload.builder().commands(new Join(1, servers(1, 1))).results(new Ok()).build());
  ...
  /**
   * Runs a single-client, multi-group search. The client should be client(1). Must call
   * setupStates(2, 1, 1, N) before calling this method.
   */
  void singleClientMultiGroupSearch() {
    ...
    initSearchState.addClientWorker(
        CCA,
        Workload.builder()
            .commands(new Join(1, servers(1, 1)), new Join(2, servers(2, 1)), new Leave(1))
    ...
    searchSettings
        ...
        .clearGoals()
        .addGoal(clientHasResults(client(1), 1));
    bfs(firstJoin);
    ...
    searchSettings.clearGoals().resetNetwork().addPrune(CLIENTS_DONE).maxTimeSecs(30);
    bfs(ccaDone);
  ...
  /**
   * Runs a multi-client, multi-group search. The clients should be client(1), client(2). This
   * method forces client(1)'s commands to be executed before client(2)'s. Must call setupStates(2,
   * 1, 1, N) before calling this method.
   */
  void multiClientMultiGroupSearch() {
    ...
    initSearchState.addClientWorker(
        CCA,
        Workload.builder()
            .commands(new Join(1, servers(1, 1)), new Join(2, servers(2, 1)))
            .build());
    ...
    searchSettings.clearGoals().resetNetwork().maxTimeSecs(30).addPrune(CLIENTS_DONE);
    bfs(secondJoin);
