...
public final class ShardStorePart1Test extends ShardStoreBaseTest {
  ...
  public void test03ShardsMoveOnJoin() throws InterruptedException {
    ...
    for (String key : kv.keySet()) {
      runState.addClientWorker(client(i), KVStoreWorkload.workload(get(key)));
      i++;
    }
  ...
  public void test04ShardsMoveOnMove() throws InterruptedException {
    ...
    assertEquals(group2Shards, Sets.union(config1.groupInfo().get(2).getRight(), toMove));
    ...
    for (String key : kv.keySet()) {
      runState.addClientWorker(
          ...
          KVStoreWorkload.workload(
              Collections.singletonList(get(key)),
              Collections.singletonList(getResult(kv.get(key)))));

      if (group2Shards.contains(ShardStoreNode.keyToShard(key, numShards))) {
    ...
    // Count number of keys gotten
    runSettings.addInvariant(RESULTS_OK);
    runSettings.addInvariant(
        StatePredicate.statePredicateWithMessage(
            "Only group 2 operations completed",
            s -> {
              for (Entry<Address, List<Result>> e : s.results().entrySet()) {
                Address a = e.getKey();
                if (!group2Clients.contains(a) && !group1Clients.contains(a)) {
                  continue;
                }

                if (e.getValue().isEmpty() && group2Clients.contains(a)) {
                ...
                if (!e.getValue().isEmpty() && group1Clients.contains(a)) {
                  return new ImmutablePair<>(
                      false,
                      String.format("%s is a client of group 1 but could complete operation", a));
                }
              }
  ...
  public void test06RepeatedPartitioning() throws InterruptedException {
    ...
    for (int i = 1; i <= nClients; i++) {
      runState.addClientWorker(client(i), KVStoreWorkload.differentKeysInfiniteWorkload(10), false);
    }
  ...
  public void test07ConstantMovement() throws InterruptedException {
    ...
    // Startup the clients
    for (int i = 1; i <= nClients; i++) {
      runState.addClientWorker(client(i), KVStoreWorkload.differentKeysInfiniteWorkload, false);
    }
  ...
  public void test10SingleClientSingleGroupSearch() {
    ...
    initSearchState.addClientWorker(client(1), KVStoreWorkload.putGetWorkload);
  ...
  public void test12MultiClientMultiGroupSearch() {
    ...
    Workload w1 =
        KVStoreWorkload.builder()
            .commands(append("foo-1", "X1"), append("foo-2", "X2"))
            .results(appendResult("X1"), appendResult("X2"))
            .build();
    initSearchState.addClientWorker(client(1), w1);

    Workload w2 =
        KVStoreWorkload.builder()
            .commands(append("foo-1", "Y1"), append("foo-2", "Y2"))
            .results(appendResult("X1Y1"), appendResult("X2Y2"))
            .build();
  ...
  private void randomSearch(int numServersPerGroup) {
    ...
    Workload w1 =
        KVStoreWorkload.builder().commands(append("foo-1", "X"), append("foo-1", "Y")).build();
    initSearchState.addClientWorker(client(1), w1);

    Workload w2 = KVStoreWorkload.builder().commands(append("foo-1", "Z")).build();
    initSearchState.addClientWorker(client(2), w2);

    Workload w3 =
        KVStoreWorkload.builder().commands(append("foo-2", "X"), append("foo-2", "Y")).build();
    initSearchState.addClientWorker(client(3), w3);

    Workload w4 = KVStoreWorkload.builder().commands(append("foo-2", "Z")).build();
