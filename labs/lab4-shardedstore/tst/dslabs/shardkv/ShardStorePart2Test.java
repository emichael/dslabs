...
public class ShardStorePart2Test extends ShardStoreBaseTest {
  ...
  public void test01SingleBasic() throws InterruptedException {
    int numGroups = 1, numServersPerGroup = 3, numShardMasters = 3, numShards = 2;
    ...
    runState.addClientWorker(client(1), TransactionalKVStoreWorkload.simpleWorkload);
  ...
  public void test02MultiBasic() throws InterruptedException {
    int numGroups = 2, numServersPerGroup = 3, numShardMasters = 3, numShards = 2;
  ...
  public void test03NoProgress() throws InterruptedException {
    ...
    sendCommandAndCheck(client, multiPut("key1-1", "foo1", "key1-2", "foo2"), multiPutOk());

    // Let the previous transaction result propagate
    Thread.sleep(1000);

    // Client can talk to both groups, but they can't talk to each other
    runSettings.partition(servers(1, numServersPerGroup), servers(2, numServersPerGroup));
    for (int g = 1; g <= 2; g++) {
    ...
    sendCommandAndCheck(client, multiPut("key2-1", "foo1", "key3-1", "foo2"), multiPutOk());
    sendCommandAndCheck(client, multiPut("key2-2", "foo1", "key3-2", "foo2"), multiPutOk());
  ...
  public void test04PutGetIsolation() throws InterruptedException {
    int numGroups = 2, numServersPerGroup = 3, numShardMasters = 3, numShards = 2, numRounds = 100;
    ...
    runState.addClientWorker(
        client(1),
        TransactionalKVStoreWorkload.builder()
            .commandStrings("MULTIPUT:key%i#1:foo%i:key%i#2:foo%i")
            .resultStrings(OK)
            .numTimes(numRounds)
            .build());
    runState.addClientWorker(
        client(2),
        TransactionalKVStoreWorkload.builder()
            .commandStrings("MULTIGET:key%i#1:key%i#2")
            .numTimes(numRounds)
            .build());
    ...
    runSettings
        ...
        .addInvariant(resultsHaveType(client(2), MultiGetResult.class))
        .addInvariant(MULTI_GETS_MATCH);
  }

  private void repeatedPutsGetsInternal(boolean moveShards) throws InterruptedException {
    int numGroups = 3,
        numServersPerGroup = 3,
        numShardMasters = 3,
        numShards = 10,
        testLengthSecs = 50,
        nClients = 5;
    ...
    for (int i = 1; i <= nClients; i++) {
      runState.addClientWorker(
          client(i), TransactionalKVStoreWorkload.differentKeysInfiniteWorkload(numShards), false);
    }
  ...
  @Test(timeout = 60 * 1000)
  @TestDescription("Repeated MultiPuts and MultiGets, different keys; constant movement")
  @Category({RunTests.class, UnreliableTests.class})
  @TestPointValue(20)
  ...
  public void test08SingleClientSingleGroupSearch() {
    ...
    initSearchState.addClientWorker(client(1), TransactionalKVStoreWorkload.putGetWorkload);
  ...
  public void test10MultiClientMultiGroupSearch() {
    ...
    Workload w1 =
        TransactionalKVStoreWorkload.builder()
            .commands(multiPut("foo-1", "X", "foo-2", "Y"), swap("foo-1", "foo-2"))
  ...
  private void randomSearch(int numServersPerGroup) {
    ...
    Workload ccWorkload =
        Workload.builder()
            .commands(
                ...
                new Join(2, servers(2, numServersPerGroup)),
                new Leave(1))
    ...
    initSearchState.addClientWorker(
        client(1),
        TransactionalKVStoreWorkload.builder()
            .commands(multiPut("foo-1", "X", "foo-2", "Y"))
            .results(multiPutOk())
            .build());

    initSearchState.addClientWorker(
        client(2),
        TransactionalKVStoreWorkload.builder().commands(multiGet("foo-1", "foo-2")).build());
    ...
    searchSettings
        ...
        .addInvariant(
            statePredicateWithMessage(
                "MultiGet returns correct results",
                s -> {
                  ...
                  if (results.size() > 1) {
                    return new ImmutablePair<>(
                        false, String.format("%s received multiple MultiGetResults", client(2)));
                  }
                  Result r = results.get(0);
                  if (!Objects.equals(r, multiGetResult("foo-1", "X", "foo-2", "Y"))
                      && !Objects.equals(
                          r, multiGetResult("foo-1", KEY_NOT_FOUND, "foo-2", KEY_NOT_FOUND))) {
                    return new ImmutablePair<>(
                        false,
                        String.format(
                            "%s matches neither of %s or %s",
                            r,
                            multiGetResult("foo-1", KEY_NOT_FOUND, "foo-2", KEY_NOT_FOUND),
