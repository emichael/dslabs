...
public abstract class TransactionalKVStoreWorkload extends KVStoreWorkload {
  public static MultiGet multiGet(@NonNull Object... keys) {
    return new MultiGet(Arrays.stream(keys).map(Object::toString).collect(Collectors.toSet()));
  }
  ...
  public static MultiGetResult multiGetResult(@NonNull Map<String, String> values) {
  ...
  public static class TransactionalKVStoreParser
  ...
  public static final Workload simpleWorkload =
      builder()
          .commands(
              multiPut("key1-1", "foo1", "key1-2", "foo2"),
              multiGet("key1-1", "key1-2"),
              append("key1-1", "bar1"),
              append("key1-2", "bar2"),
              multiGet("key1-1", "key1-2"),
              swap("key1-1", "key1-2"),
          ...
          .results(
              ...
              appendResult("foo1bar1"),
              appendResult("foo2bar2"),
              ...
              multiGetResult(
                  "key1-1", "foo2bar2", "key2-1", "baz1", "key3-1", MultiGetResult.KEY_NOT_FOUND))
          .build();

  public static final Workload putGetWorkload =
      builder()
          .commands(multiPut("key1", "foo1", "key2", "foo2"), multiGet("key1", "key2"))
          .results(multiPutOk(), multiGetResult("key1", "foo1", "key2", "foo2"))
          .build();
  ...
  private static class DifferentKeysInfiniteWorkload extends InfiniteWorkload {
    ...
    DifferentKeysInfiniteWorkload(int numShards) {
      ...
      this.shardNums = IntStream.rangeClosed(1, numShards).boxed().collect(Collectors.toList());
    }
    ...
    public Pair<Command, Result> nextCommandAndResult(Address clientAddress) {
      ...
      for (int i = 0; i < numKeys; i++) {
        keys.add(String.format("key-%s-%s", clientAddress, shardNums.get(i)));
      }

      if (lastWasGet) {
        Map<String, String> puts = new HashMap<>();
        keys.forEach(s -> puts.put(s, RandomStringUtils.random(8, 0, 0, true, true, null, rand)));
        data.putAll(puts);
        lastWasGet = false;
        return new ImmutablePair<>(multiPut(puts), multiPutOk());
      } else {
        Map<String, String> values = new HashMap<>();
        keys.forEach(k -> values.put(k, data.getOrDefault(k, MultiGetResult.KEY_NOT_FOUND)));
        lastWasGet = true;
        return new ImmutablePair<>(multiGet(keys), multiGetResult(values));
      }
    }
  ...
  public static final StatePredicate MULTI_GETS_MATCH =
      resultsPredicateWithMessage(
          "Multi-get returns same values for all keys",
          rs -> {
            for (List<Result> r : rs) {
              for (Result result : r) {
                ...
                if (mgr.values().values().stream().distinct().count() != 1) {
                  return new ImmutablePair<>(
                      false, String.format("%s has multiple distinct values", mgr));
                }
              }
            }
