...
public class PaxosTest extends BaseJUnitTest {
  ...
  private static StatePredicate hasStatus(Address a, int i, PaxosLogSlotStatus s) {
  ...
  private static StatePredicate hasCommand(Address a, int i, Command c) {
    return StatePredicate.statePredicate(
        ...
        st -> Objects.equals(((PaxosServer) st.server(a)).command(i), c));
  }

  /**
   * Checks basic validity of the values returned by firstNonCleared and lastNonEmpty. This is
   * included in the log consistency invariants and shouldn't be included by itself.
   */
  private static final StatePredicate MARKERS_VALID =
      StatePredicate.statePredicateWithMessage(
          "First non-cleared and last non-empty valid",
          st -> {
            for (Node n : st.servers()) {
              ...
              if (nc < 1) {
                return new ImmutablePair<>(
                    false, String.format("%s returned %s as first non-cleared slot", a, nc));
              }

              if (ne < 0) {
                return new ImmutablePair<>(
                    false, String.format("%s returned %s as last non-empty slot", a, ne));
              }
              ...
              if (ne > 0 && p.status(ne) == EMPTY) {
                return new ImmutablePair<>(
                    false,
                    String.format(
                        "%s returned %s as last non-empty slot, but slot has status empty", a, ne));
              }
  ...
  private static Pair<Boolean, String> slotValid(AbstractState st, int i) {
    ...
    for (Node n : st.servers()) {
      ...
      if (i < nc && s != CLEARED) {
        return new ImmutablePair<>(
            false,
            String.format(
                "%s has status %s for slot %s but the firstNonCleared slot is %s", a, s, i, nc));
      }
      ...
      if (i > ne && s != EMPTY) {
        return new ImmutablePair<>(
            false,
            String.format(
                "%s has status %s for slot %s but the lastNonEmpty slot is %s", a, s, i, ne));
      }

      if ((s == EMPTY || s == CLEARED) && c != null) {
        return new ImmutablePair<>(
            false,
            String.format("%s has status %s for slot %s but returned command %s", a, s, i, c));
      }

      if (c instanceof AMOCommand) {
        return new ImmutablePair<>(
            false, String.format("%s returned an AMOCommand for slot %s", a, i));
      }
      ...
      if (s == CHOSEN) {
        if (isChosen && !Objects.equals(chosen, c)) {
          ...
          return new ImmutablePair<>(
              false,
              String.format("Two different commands (%s and %s) chosen for slot %s", chosen, c, i));
        }
        chosen = c;
        isChosen = true;
      }
    ...
    for (Node n : st.servers()) {
      ...
      if (s != EMPTY && (s != ACCEPTED || !isChosen || Objects.equals(chosen, c))) {
        count++;
      }
    }
    ...
    if (2 * count <= st.numServers()) {
      if (isChosen) {
        return new ImmutablePair<>(
            false, String.format("%s chosen for slot %s without a majority accepting", chosen, i));
      }

      return new ImmutablePair<>(
          false, String.format("Slot %s cleared without a majority accepting", i));
    }
  ...
  private static StatePredicate slotValid(int i) {
    return StatePredicate.statePredicateWithMessage(
        "Logs consistent for slot " + i, st -> slotValid(st, i));
  }

  /** Checks that all non-empty, non-cleared log slots are valid. */
  private static final StatePredicate LOGS_CONSISTENT =
      StatePredicate.statePredicateWithMessage(
              "Active log slots consistent",
              st -> {
                int minNonCleared = Integer.MAX_VALUE, maxNonEmpty = 0;

                for (Node n : st.servers()) {
                  PaxosServer p = (PaxosServer) n;
                  minNonCleared = Math.min(minNonCleared, p.firstNonCleared());
  ...
  /** Checks that all non-empty slots log slots are valid. Also checks validity of markers. */
  private static final StatePredicate LOGS_CONSISTENT_ALL_SLOTS =
      StatePredicate.statePredicateWithMessage(
              "Non-empty log slots consistent",
              st -> {
                int maxNonEmpty = 0;
  ...
  public void test11ClearsMemory() throws InterruptedException {
    ...
    for (int i = 0; i < iters; i++) {
      for (int key = 0; key < items; key++) {
        sendCommandAndCheck(client, put(key, RandomStringUtils.randomAscii(valueSize)), putOk());
      }
    }
    ...
    System.out.println("Using " + readableSize(afterPutBytes) + " after puts.");
  ...
  @Category({RunTests.class, UnreliableTests.class})
  @TestPointValue(15)
  public void test14SynchronousClientsUnreliable() throws InterruptedException {
    runSettings.networkDeliverRate(0.8);
  ...
  @Category({RunTests.class, UnreliableTests.class})
  @TestPointValue(15)
  public void test15ConcurrentAppendsUnreliable() throws InterruptedException {
    runSettings.networkDeliverRate(0.8);
  ...
  public void test16SinglePartition() throws InterruptedException {
    ...
    List<Address> partition = Lists.newArrayList(server(1), server(2), server(3));
    for (int i = 1; i <= nClients; i++) {
  ...
  public void test17ConstantRepartition() throws InterruptedException {
    ...
    setupStates(nServers);
    for (int i = 1; i <= nClients; i++) {
      runState.addClientWorker(client(i), differentKeysInfiniteWorkload(10), false);
    }
  ...
  @Category({RunTests.class, UnreliableTests.class})
  @TestPointValue(30)
  public void test18ConstantRepartitionUnreliable() throws InterruptedException {
    runSettings.networkDeliverRate(0.8);
  ...
  public void test19RepartitionFullThroughput() throws InterruptedException {
    ...
    for (int i = 1; i <= nClients; i++) {
      ...
      runState.addClientWorker(client(i + nClients), appendDifferentKeyWorkload(nRounds));
    }
  ...
  public void test22TwoClientsSearch() {
    ...
    initSearchState.addClientWorker(
        client(1),
        Workload.builder().commands(append("foo", "X")).results(appendResult("X")).build());
    initSearchState.addClientWorker(
        client(2),
        Workload.builder().commands(append("foo", "Y")).results(appendResult("XY")).build());
    ...
    searchSettings.resetNetwork().partition(server(2), server(3), client(2));
    bfs(firstAppendSent);
    assertGoalFound();
    ...
    searchSettings.deliverTimers(false).resetNetwork().partition(server(1), server(3), client(2));
    bfs(firstAppendSent);
  ...
  public void test23QuorumCheckingSearch() {
    ...
    initSearchState.addClientWorker(client(1), Workload.builder().commands(c1).build());
    initSearchState.addClientWorker(client(2), Workload.builder().commands(c2).build());
    ...
    searchSettings
        ...
        .nodeActive(server(1), true)
        .nodeActive(server(5), true)
        .clearDeliverTimers()
        .deliverTimers(server(4), false)
        .deliverTimers(server(3), false)
        .deliverTimers(client(1), false)
        .clearGoals()
  ...
  public void test24LogsWithHolesSearch() {
    ...
    initSearchState.addClientWorker(
        client(1),
        KVStoreWorkload.builder().commands(append("foo", "x"), append("foo", "z")).build());
    initSearchState.addClientWorker(
        client(2),
        KVStoreWorkload.builder().commands(append("foo", "y"), append("foo", "w")).build());
    ...
    for (Address a : servers(3)) {
      searchSettings.addGoal(
          hasStatus(a, 2, CHOSEN).and(hasStatus(a, 1, ACCEPTED).or(hasStatus(a, 1, EMPTY))));
    }
  ...
  private void randomSearch() {
    initSearchState.addClientWorker(
        client(1), KVStoreWorkload.builder().commands(append("foo", "x")).build());
    initSearchState.addClientWorker(
        client(2), KVStoreWorkload.builder().commands(append("foo", "y")).build());

    searchSettings
        .maxDepth(1000)
        .maxTimeSecs(20)
        .addInvariant(APPENDS_LINEARIZABLE)
        .addInvariant(LOGS_CONSISTENT)
        .addPrune(CLIENTS_DONE);
  ...
  public void test27SingletonPaxos() throws InterruptedException {
    ...
    System.out.println("Checking that 3 commands can be processed in 6 steps");
    setupStates(1);
