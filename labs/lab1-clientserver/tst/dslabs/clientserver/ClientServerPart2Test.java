...
public final class ClientServerPart2Test extends ClientServerBaseTest {
  ...
  @TestPointValue(20)
  @Category({RunTests.class, UnreliableTests.class})
  public void test02SingleClientAppendsUnreliable() throws InterruptedException {
  ...
  @TestPointValue(20)
  @Category({RunTests.class, UnreliableTests.class})
  public void test03MultiClientDifferentKeyUnreliable() throws InterruptedException {
  ...
  @TestPointValue(20)
  @Category({RunTests.class, UnreliableTests.class})
  public void test04MultiClientSameKeyUnreliable() throws InterruptedException {
  ...
  public void test05GarbageCollection() throws InterruptedException {
    ...
    System.out.println("Using " + readableSize(afterAppendBytes) + " after appends.");
    ...
    for (int key = 0; key < items; key++) {
      for (int c = 1; c <= numClients; c++) {
        String k = String.format("client%s-key%s", c, key);
        sendCommandAndCheck(runState.client(client(c)), put(k, ""), putOk());
      }
    }
    runState.stop();
  ...
  public void test07SingleClientSearch() {
    ...
    searchSettings.addInvariant(RESULTS_OK).addGoal(CLIENTS_DONE).maxTimeSecs(10);
    bfs(initSearchState);
    assertGoalFound();
  ...
  public void test09MultiClientDifferentKeySearch() {
    ...
    for (int i = 1; i <= numClients; i++) {
      initSearchState.addClientWorker(client(i), appendDifferentKeyWorkload(numRounds));
    }
  ...
  public void test10MultiClientSameKeySearch() {
    ...
    for (int i = 1; i <= numClients; i++) {
      initSearchState.addClientWorker(
          client(i),
          KVStoreWorkload.builder().commandStrings("APPEND:foo:%i").numTimes(numRounds).build());
    }

    System.out.println("Checking that an end state is reachable");
    searchSettings.addInvariant(APPENDS_LINEARIZABLE).addGoal(CLIENTS_DONE).maxTimeSecs(30);
    bfs(initSearchState);
    assertGoalFound();
  ...
  public void test11RandomSearchInfiniteWorkloads() {
    initSearchState.addClientWorker(client(1), differentKeysInfiniteWorkload);
    ...
    initSearchState.addClientWorker(client(2), differentKeysInfiniteWorkload);
    dfs(initSearchState);
  }
}
