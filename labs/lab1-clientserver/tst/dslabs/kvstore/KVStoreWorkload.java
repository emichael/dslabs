...
public abstract class KVStoreWorkload extends Workload {
  ...
  public static class KVStoreParser
      implements SerializableFunction<Pair<String, String>, Pair<Command, Result>> {
    @Override
    public Pair<Command, Result> apply(Pair<String, String> commandAndResultString) {
      return parse(commandAndResultString);
    }
  }

  public static Pair<Command, Result> parse(@NonNull Pair<String, String> commandAndResultString) {
  ...
  public static final Workload simpleWorkload =
      builder()
          .commands(
              put("key1", "v1a"),
              get("key1"),
  ...
  public static final Workload putAppendGetWorkload =
      builder()
          .commands(put("foo", "bar"), append("foo", "baz"), get("foo"))
          .results(putOk(), appendResult("barbaz"), getResult("barbaz"))
          .build();

  public static final Workload appendAppendGet =
      builder()
          .commands(append("foo", "bar"), append("foo", "bar"), get("foo"))
          .results(appendResult("bar"), appendResult("barbar"), getResult("barbar"))
          .build();

  public static final Workload putGetWorkload =
      builder().commands(put("foo", "bar"), get("foo")).results(putOk(), getResult("bar")).build();
  ...
  public static Workload appendDifferentKeyWorkload(int numRounds) {
    ...
    return builder()
        .commandStrings(workload.toArray(new String[0]))
        .resultStrings(results.toArray(new String[0]))
        .build();
  }

  public static Workload appendSameKeyWorkload(int numRounds) {
    return builder().commandStrings("APPEND:foo:%a,%i").numTimes(numRounds).build();
  }

  private static class DifferentKeysInfiniteWorkload extends InfiniteWorkload {
    ...
    public Pair<Command, Result> nextCommandAndResult(Address clientAddress) {
      ...
      if (lastWasGet) {
        lastPutKey = clientAddress.toString() + "-" + (rand.nextInt(5) + 1);
        String v = RandomStringUtils.random(8, 0, 0, true, true, null, rand);
        data.put(lastPutKey, v);
        lastWasGet = false;
        return new ImmutablePair<>(put(lastPutKey, v), putOk());
      } else {
        lastWasGet = true;
        return new ImmutablePair<>(get(lastPutKey), getResult(data.get(lastPutKey)));
      }
    }
  ...
  public static final Workload differentKeysInfiniteWorkload = new DifferentKeysInfiniteWorkload();

  public static Workload differentKeysInfiniteWorkload(int millisBetweenRequests) {
  ...
  /**
   * Tests whether the results a group of clients get back from sending appends to the same key are
   * linearizable. Assumes that the values being appended are non-empty.
   *
   * @param clientWorkers the clients sending the appends; if this is null, the predicate uses all
   *     ClientWorkers
   */
  private static StatePredicate appendsLinearizableInternal(Iterable<Address> clientWorkers) {
    return statePredicateWithMessage(
        "Sequence of appends to the same key is linearizable",
        s -> {
          List<String> allResults = new ArrayList<>();

          for (Address a : (clientWorkers == null ? s.clientWorkerAddresses() : clientWorkers)) {
            ...
            while (cs.hasNext() && rs.hasNext()) {
              ...
              if (!(c instanceof Append)) {
                throw new RuntimeException("Client workers have non-Append Commands");
              }

              if (!(r instanceof AppendResult)) {
                return new ImmutablePair<>(
                    false, String.format("%s got %s as result for %s", a, r, c));
              }

              Append append = (Append) c;
              AppendResult appendResult = (AppendResult) r;

              if (!appendResult.value().endsWith(append.value())) {
                return new ImmutablePair<>(
                    false, String.format("%s got %s as result for %s", a, r, c));
              }
          ...
          for (int i = 0; i < allResults.size() - 1; i++) {
            if (!allResults.get(i + 1).startsWith(allResults.get(i))
                || allResults.get(i + 1).equals(allResults.get(i))) {
              return new ImmutablePair<>(
                  false,
                  String.format(
                      "%s is inconsistent with %s",
                      appendResult(allResults.get(i)), appendResult(allResults.get(i + 1))));
            }
          }
  ...
  public static final StatePredicate APPENDS_LINEARIZABLE = appendsLinearizableInternal(null);
}
