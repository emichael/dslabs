...
public final class PingTest extends BaseJUnitTest {
  ...
  private static final class PingParser
      implements SerializableFunction<Pair<String, String>, Pair<Command, Result>> {
    @Override
    public Pair<Command, Result> apply(@NonNull Pair<String, String> commandAndResultString) {
      return new ImmutablePair<>(
          new Ping(commandAndResultString.getValue()), new Pong(commandAndResultString.getValue()));
    }
  }
  ...
  static Workload repeatedPings(int numPings) {
    return Workload.builder()
        ...
        .commandStrings("ping-%i")
        .resultStrings("ping-%i")
        .numTimes(numPings)
        .build();
  }
  ...
  public void test01BasicPing() throws InterruptedException {
    Workload workload =
        Workload.builder()
            .commands(new Ping("Hello, World!"))
            .results(new Pong("Hello, World!"))
            .build();
  ...
  public void test04PingSearch() throws InterruptedException {
    ...
    System.out.println("Checking that all of the returned pongs match pings");
