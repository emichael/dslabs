...
public class PrimaryBackupTest extends BaseJUnitTest {
  ...
  private static StatePredicate hasViewReply(final int viewNum) {
    return StatePredicate.containsMessageMatching(
        "ViewReply with viewNum: " + viewNum,
        m -> (m instanceof ViewReply) && ((ViewReply) m).view().viewNum() >= viewNum);
  }

  private static StatePredicate hasViewReply(
      final int viewNum, final Address primary, final Address backup) {
    View v = new View(viewNum, primary, backup);
    return StatePredicate.containsMessageMatching(
        "ViewReply with " + v, m -> (m instanceof ViewReply) && ((ViewReply) m).view().equals(v));
  }

  /* Search Test Helper Methods */

  /**
   * Takes the initial search state (after nodes have been added), and finds the state where the
   * required view has started, and all clients know about the state.
   */
  private SearchState initView(
      SearchState startState, int viewNum, Address primary, Address backup, Address... clients) {
    ...
    StatePredicate viewRepliesSent =
        statePredicate(
            String.format("ViewReply for %s sent to nodes %s, primary ack sent", toStart, toInit),
            s -> {
              ...
              for (MessageEnvelope me : s.network()) {
                ...
                if (m instanceof Ping
                    && me.from().equals(primary)
                    && ((Ping) m).viewNum() == toStart.viewNum()) {
                  ackFound = true;
                } else if (m instanceof ViewReply && ((ViewReply) m).view().equals(toStart)) {
    ...
    temp.maxTimeSecs(30)
        .outputFreqSecs(-1)
        .addPrune(hasViewReply(viewNum + 1))
        .addPrune(hasViewReply(viewNum).and(hasViewReply(viewNum, primary, backup).negate()))
        .networkActive(false)
        .nodeActive(VSA, true)
        .addGoal(viewRepliesSent.and(hasViewReply(viewNum + 1).negate()));
    if (backup != null) {
      temp.linkActive(primary, backup, true).linkActive(backup, primary, true);
    }
    ...
    for (Address a : toInit) {
      current =
          current.stepMessage(new MessageEnvelope(VSA, a, new ViewReply(toStart)), null, false);
    }

    // Deliver the Ack from the primary
    current =
        current.stepMessage(
            new MessageEnvelope(primary, VSA, new Ping(toStart.viewNum())), null, false);

    System.out.println("View initialized.\n");
    return current;
  }

  private SearchState initView(Address primary, Address backup, Address... clients) {
    return initView(
        initSearchState,
        backup == null ? INITIAL_VIEWNUM : INITIAL_VIEWNUM + 1,
        primary,
        backup,
        clients);
  }
  ...
  /**
   * Takes the running state and polls the view server for the current view. It uses runState in the
   * class, which must currently be running.
   *
  ...
  /**
   * Waits until a view becomes active, or for 4 ping checks to elapse, whichever comes first. If
   * the expected view is not returned, throws an error.
   *
   * @param primary the primary expected in the view (non-null)
   * @param backup the backup expected for the view (potentially null)
   */
  private void waitForView(Address primary, Address backup) throws InterruptedException {
    for (int i = 0; i < 4; i++) {
      View v = getView();
      if (Objects.equals(primary, v.primary()) && Objects.equals(backup, v.backup())) {
        return;
      }
    ...
    View v = getView();
    if (!(Objects.equals(primary, v.primary()) && Objects.equals(backup, v.backup()))) {
      fail(String.format("Expected view primary: %s, backup: %s did not start", primary, backup));
    }
  }
   ...
   *
   * @param primary the primary to install
   * @param backup the backup to install
   */
  private void setupRunView(Address primary, Address backup) throws InterruptedException {
  ...
  public void test05MaxViewServerPingsCount() throws InterruptedException {
    ...
    double allowed = ((double) (t2 - t1)) / PING_MILLIS * runState.numNodes() * 2;

    if (received > allowed) {
      fail("Too many ViewServer messages: " + received + " (expected <=" + allowed + ")");
    }
  }
  ...
  public void test10ConcurrentPut() throws InterruptedException {
    ...
    for (int i = 1; i <= nClients; i++) {
      ...
      runState.addClientWorker(client(i), KVStoreWorkload.workload(workload));
    }
    ...
    Workload readKeys =
        Workload.builder()
            .commands(
                IntStream.range(0, nKeys)
                    .mapToObj(KVStoreWorkload::get)
                    .collect(Collectors.toList()))
            .build();
    runState.addClientWorker(new LocalAddress("client-readprimary"), readKeys);
    runState.run(runSettings);
    ...
    runState.addClientWorker(new LocalAddress("client-readbackup"), readKeys);
  ...
  public void test11ConcurrentAppend() throws InterruptedException {
    ...
    for (int i = 1; i <= nClients; i++) {
      runState.addClientWorker(client(i), appendSameKeyWorkload(nAppends));
    }
  ...
  public void test14RepeatedCrashes() throws InterruptedException {
    ...
    for (int i = 0; i < nClients; i++) {
      runState.addClientWorker(client(i), differentKeysInfiniteWorkload, false);
    }
  ...
  public void test15RepeatedCrashesUnreliable() throws InterruptedException {
    runSettings.networkDeliverRate(0.8).nodeUnreliable(VSA, false).nodeUnreliable(TA, false);
  ...
  public void test16SingleClientSearch() {
    ...
    searchSettings.addInvariant(RESULTS_OK).addGoal(CLIENTS_DONE).maxTimeSecs(30);
    bfs(initSearchState);
    assertGoalFound();
  ...
  public void test17SingleClientMultiServerSearch() {
    ...
    final SearchState viewInitializedState = initView(server(1), server(2), client(1));
  ...
  public void test18MultiClientWritesVisibleSearch() {
    ...
    initSearchState.addClientWorker(client(1), Workload.workload(append("foo", "x")));
    initSearchState.addClientWorker(client(2), Workload.workload(append("foo", "y")));

    // Get a state w/ primary and backup initialized
    final SearchState viewInitialized = initView(server(1), server(2), client(1), client(2));
    ...
    searchSettings
        ...
        .addGoal(
            statePredicate(
                "Both clients sent messages to primary",
                s ->
                    Streams.stream(s.network())
    ...
    final Map<Address, Set<MessageEnvelope>> sentMessages =
        Streams.stream(requestsSent.network().iterator())
            .filter(e -> e.to().equals(server(1)))
            .filter(e -> senders.contains(e.from()))
            .collect(Collectors.toMap(MessageEnvelope::from, Sets::newHashSet, Sets::union));
    ...
    for (Address sender : Lists.reverse(senders)) {
      ...
      for (MessageEnvelope me : pToB.get(sender)) {
        forwardedReversed = forwardedReversed.stepMessage(me, null, false);
    ...
    searchSettings.clear().addInvariant(APPENDS_LINEARIZABLE).addGoal(CLIENTS_DONE).maxTimeSecs(20);
    ...
    searchSettings
        ...
        .addPrune(CLIENTS_DONE)
        .addPrune(hasViewReply(INITIAL_VIEWNUM + 3))
        .addPrune(hasViewReply(INITIAL_VIEWNUM + 2, server(1), null))
        .maxTimeSecs(30);
    bfs(forwardedReversed);
    ...
    for (MessageEnvelope me : pToB.get(client(2))) {
      onlySecondForwarded = onlySecondForwarded.stepMessage(me, null, false);
  ...
  public void test19MultipleFailuresSearch() {
    ...
    initSearchState.addClientWorker(
        client(1),
        KVStoreWorkload.builder().commands(append("foo", "x")).results(appendResult("x")).build());
    initSearchState.addClientWorker(
        client(2),
        KVStoreWorkload.builder().commands(append("foo", "y")).results(appendResult("xy")).build());

    // Add primary and fail to it
    final SearchState firstView =
        initView(initSearchState, INITIAL_VIEWNUM + 1, server(1), server(2));
    final SearchState primaryAlone =
        initView(firstView, INITIAL_VIEWNUM + 2, server(1), null, client(1));

    // Have the client commit the operation to only the primary
    searchSettings
        .maxTimeSecs(10)
        .partition(server(1), client(1), VSA)
        .addInvariant(RESULTS_OK)
    ...
    searchSettings
        ...
        .clearPrunes()
        .addPrune(
            hasViewReply(INITIAL_VIEWNUM + 3)
                .implies(hasViewReply(INITIAL_VIEWNUM + 3, server(1), server(2)))
                .negate())
        .addPrune(
            hasViewReply(INITIAL_VIEWNUM + 4)
  ...
  public void test20RandomSearch() {
    ...
    initSearchState.addClientWorker(
        client(1),
        KVStoreWorkload.builder().commands(append("foo", "w"), append("foo", "x")).build());
    initSearchState.addClientWorker(
        client(2),
        KVStoreWorkload.builder().commands(append("foo", "y"), append("foo", "z")).build());
