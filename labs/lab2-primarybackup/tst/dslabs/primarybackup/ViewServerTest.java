...
public class ViewServerTest extends DSLabsJUnitTest {
  static final Address VSA = new LocalAddress("viewserver"), TA = new LocalAddress("testserver");
  ...
  /**
   * Sets up initial view with primary and backup. Should only be called at the beginning of a test.
   *
   * @param primary the address to make primary
   * @param backup the address to make backup (possibly null)
   * @param ackView whether or not to acknowledge the view
   */
   ...
   *
   * @param serversSendingPings servers to send pings every interval.
   */
  ...
  public void test12NewViewNotStarted() {
    ...
    timeoutFully();
    View v = getView();
    if (Objects.equals(v.primary(), server(1)) && Objects.equals(v.backup(), server(2))) {
